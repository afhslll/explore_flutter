...
class _LiveLocationScreenState extends State<LiveLocationScreen> {
  ...
  Future<Uint8List> getMarker() async {
    final ByteData byteData =
        await DefaultAssetBundle.of(context).load(ConstantAssetsString.carIcon);
    return byteData.buffer.asUint8List();
  }

  void updateMarkerAndCircle(LocationData newLocalData, Uint8List imageData) {
    final LatLng latlng = LatLng(newLocalData.latitude, newLocalData.longitude);
    setState(() {
      marker = Marker(
  ...
  Future<void> getCurrentLocation() async {
    try {
      ...
      _locationSubscription =
        ...
        print(
            'Current location ${newLocalData.latitude} ${newLocalData.longitude}');
        if (_controller != null) {
